...
export function SavingIndicator({ status, className }: SavingIndicatorProps) {
  ...
  return (
    <div
      ...
      aria-live="polite"
      aria-atomic="true"
      aria-label={status === "saving" ? "Saving changes" : status === "saved" ? "Changes saved" : status === "error" ? "Error saving changes" : undefined}
      className={cn(
      ...
      <div className="flex items-center justify-center w-3 h-3 relative shrink-0">
        ...
        {status === "error" && (
          <svg className="w-3.5 h-3.5 text-destructive animate-in shake duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
