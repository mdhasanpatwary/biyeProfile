...
export function BiodataCard({ username, data }: BiodataCardProps) {
  ...
  const name = basicInfo?.fullName || (language === "bn" ? "সদস্য" : "Member")

  const ageUnit = language === "bn" ? "বছর" : "Yrs"
      ...
      <div className="flex flex-col sm:flex-row items-center gap-8 md:gap-12 py-10 md:py-12 px-6 sm:px-0">
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-12 items-center w-full text-center sm:text-left">
          <div className="md:col-span-2">
            <p className="text-[10px] font-mono font-black text-foreground-muted uppercase tracking-[0.4em] mb-2">
              {language === "bn" ? "পরিচয়" : "Identification"}
            </p>
            ...
            <div className="flex items-center justify-center sm:justify-start gap-4 mt-4 md:hidden">
              ...
              <span className="text-[10px] font-mono text-foreground-muted uppercase tracking-widest">{age} {ageUnit}</span>
            </div>
          </div>

          <div className="hidden md:block">
            <p className="text-[10px] font-mono font-black text-foreground-muted uppercase tracking-[0.4em] mb-2">
              {language === "bn" ? "বিবরণ" : "Metrics"}
            </p>
            <div className="flex flex-col gap-1">
              <span className="text-[11px] font-black text-foreground uppercase tracking-widest">{religion}</span>
              <span className="text-[11px] font-mono text-foreground-muted uppercase tracking-widest">{age} {ageUnit}</span>
            </div>
          </div>

          <div className="hidden md:block">
            <p className="text-[10px] font-mono font-black text-foreground-muted uppercase tracking-[0.4em] mb-2">
              {t.biodataCard.district}
            </p>
